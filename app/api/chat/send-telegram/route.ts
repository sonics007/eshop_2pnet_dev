...
export async function POST(request: Request) {
  ...
  const targetChat = settings.telegramGroupId || settings.telegramChatId;

  if (!settings.telegramBotToken || !targetChat) {
    return NextResponse.json(
      { success: false, message: 'Telegram bot alebo cieľový chat/skupina nie sú nastavené.' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (!response.ok || data?.ok === false) {
      const message = data?.description || response.statusText || 'Telegram API odmietlo požiadavku.';
    ...
    return NextResponse.json(
      { success: false, message: 'Nepodarilo sa odoslať správu do Telegramu.' },
      { status: 500 }
    );
  }
}
