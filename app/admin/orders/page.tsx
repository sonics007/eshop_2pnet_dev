...
export default function OrdersAdminPage() {
  ...
  const updateStatus = async (status: OrderStatus) => {
    ...
    try {
      ...
      setNoteDraft('');
      setMessage(`Stav objednávky ${selectedOrder.id} bol zmenený na "${orderStatusLabels[status]}".`);
    } catch (error) {
      console.error(error);
  ...
  return (
    <AdminLayout activePanel="admin-orders">
            ...
            <div className="max-h-[520px] space-y-3 overflow-y-auto pr-1">
              {orders.map((order) => (
                ...
                >
                  ...
                  <p className="text-xs">
                    {order.customer} · {orderStatusLabels[order.status] || order.status}
                  </p>
                  ...
                  <ul className="mt-3 space-y-2 text-slate-600">
                    {selectedOrder.history.map((entry, index) => (
                      <li key={`${entry.status}-${index}`} className="rounded-xl border border-slate-200 px-3 py-2">
                        ...
                        <p className="text-sm">{orderStatusLabels[entry.status] || entry.status}</p>
